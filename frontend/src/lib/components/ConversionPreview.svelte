<script lang="ts">
  ...
  import PreviewItem from "@/lib/components/PreviewItem.svelte";
  ...
  const { sourceColor, onShowNotification }: Props = $props();
  ...
  const hasValidColors = $derived(colors.filter((c) => c.value).length >= 1);
</script>
  ...
  >
    {#if hasValidColors}
      {#each colors as color (color.format)}
        {#if color.value}
          <PreviewItem color={color.value} {onShowNotification} />
        {/if}
      {/each}
    {:else}
      <img
      ...
      <p class="preview__no-preview-item-text">
        No matching css equivalent could be generated based on your supplied
        source format and source color
      </p>
    {/if}
  </div>
</div>
